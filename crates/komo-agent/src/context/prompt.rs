...
pub(crate) fn build(
    ...
    match invocation {
        InvocationContext::Main => {
            ...
            main_prompt(workspace, &tool_names, skills, has_shell)
        }
...
const SELF_BLOCK: &str = "你运行在 komo 里。cron 任务、会话与 Run、记忆、待处理的审批 / 介入、配置、\
                          skills、toolbox、Gateway 这些都是 komo 自己的状态：问到它们就用 shell \
                          直接跑 `komo <子命令>` 查，不要去翻系统 crontab、launchctl 或同名的 skill。\n\
                          常用子命令：cron list、session list、run inspect <RUN_ID>、\
                          memory search <查询词>、intervention list、config check、skills list、\
                          toolbox list、gateway status、doctor；其余看 `komo --help`。";
