...
//! | 同一人连点两次第二次得到「已决定」 | `telegram::two_clicks_and_the_second_is_told_it_was_decided`（**BUG(1)**，`#[ignore]`）；现状由 `telegram::today_the_second_click_is_told_the_approval_does_not_exist` 钉住 |
...
//! | 决定后卡片 / 消息原地更新 | `telegram::a_decision_removes_the_buttons`、`feishu::a_decision_patches_the_card`；来源会话那一份是 **BUG(2)**（`telegram::a_decision_updates_the_source_chat_too`，`#[ignore]`） |
//! | Gateway 重启后 pending 投递补发一次 | `cross::a_pending_delivery_is_resent_once_after_a_restart`（**BUG(3)**，`#[ignore]`）；现状由 `cross::today_a_restart_leaves_the_delivery_pending_until_someone_speaks` 钉住 |
