...
pub(crate) fn build(
    ...
    match invocation {
        InvocationContext::Main => main_prompt(workspace, &tool_names, skills),
        InvocationContext::Delegated(spec) => subagent_prompt(workspace, &tool_names, spec),
    }
}

/// 主 Agent 的系统提示：身份 / 工作目录 / 挂着的工具 / §5.6 的 skills 目录 / 三条行为约束。
fn main_prompt(workspace: &Path, tool_names: &str, skills: Option<&SkillCatalog>) -> String {
