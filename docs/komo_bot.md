...
**"在跑"用动的东西说，不用字。** 工具调用行首那一格与状态行前那一格，跑着的时候是一个转圈（八帧盲文点阵，每帧一列，换帧不会让后面的字左右跳），跑完了换成定住的符号：`✔` 完成 · `✖` 失败 · `?` 结果不明（§8.6 的 uncertain 既不是成功也不是失败，所以它不共用失败那个符号）· `·` 已计划未开跑。一块静止的"运行中"只在它变的那一刻有信息，之后就分不清是还在跑还是卡住了。帧由**时钟**算而不是帧计数器：状态机不读时钟，同一时刻画出来永远是同一帧，测试给一个固定时刻就能断言它转到哪了；交给回滚区的行不给钟，因而不会定格在某一帧随机的样子上。
